...
T.Flow, T.Choice, T.Branch, T.Parallel, T.Thread, T.Loop, T.Group, T.Flow, T.Start, T.End;
var E = { nodeDefaultSize: {
	width: 260,
	height: 68
} };
T.Task, E.nodeDefaultSize.width, E.nodeDefaultSize.height, T.Flow, T.Start, T.End;
...
var O = Object.keys(D).reduce((e, t) => (e[D[t]] = t, e), {});
var te = Object.keys(D).map((e) => ({
	value: D[e],
	label: e
}));
var k = {
...
var ne = Object.keys(k).reduce((e, t) => (e[k[t]] = t, e), {});
var A = {
	requiredPrompt: "请输入...",
	iteratorSource: "迭代源",
...
var ce = class {
	constructor(e = {}) {
		this.separator = e.separator || ".", this.path = [], this.siblingStacks = [], this._pathStringCache = null, this._view = new se(this), this._keptAttrs = [];
	}
...
function le(e) {
	return N(e).replace(/--/g, "- -").replace(/--/g, "- -").replace(/-$/, "- ");
}
function P(e) {
	return N(e).replace(/\]\]>/g, "]]]]><![CDATA[>");
}
function ue(e) {
	return N(e).replace(/"/g, "&quot;").replace(/'/g, "&apos;");
}
var de = ":A-Za-z_À-ÖØ-öø-˿Ͱ-ͽͿ-҆҈-῿‌-‍⁰-↏Ⰰ-⿯、-퟿豈-﷏ﷰ-�";
var fe = de + "\\-\\.\\d·̀-ͯ‿-⁀";
...
function Ee(e, t) {
	...
	if (t.stopNodes && Array.isArray(t.stopNodes)) for (let e = 0; e < t.stopNodes.length; e++) {
		let n = t.stopNodes[e];
		typeof n == "string" ? r.push(new M(n)) : n instanceof M && r.push(n);
	}
...
function De(e, t, n, r, i, a) {
	...
	for (let c = 0; c < e.length; c++) {
		...
		if (d === t.textNodeName) {
			let e = l[u];
			p || (e = t.tagValueProcessor(d, e), e = Pe(e, t)), e = N(e), s && (o += n), o += e, s = !1, r.pop();
			continue;
		}
		if (d === t.cdataPropName) {
			s && (o += n);
			let e = l[u][0][t.textNodeName], i = P(e);
			o += `<![CDATA[${i}]]>`, s = !1, r.pop();
			continue;
		}
...
function ke(e, t) {
	if (!Array.isArray(e)) return e == null ? "" : N(e);
	let n = "";
	for (let r = 0; r < e.length; r++) {
		let i = e[r], a = je(i);
		if (a === t.textNodeName) n += N(i[a]);
...
function Ae(e, t) {
	...
	if (e && !t.ignoreAttributes) for (let r in e) {
		...
		let i = e[r];
		i === !0 && t.suppressBooleanAttributes ? n += ` ${r.substr(t.attributeNamePrefix.length)}` : n += ` ${r.substr(t.attributeNamePrefix.length)}="${ue(i)}"`;
	}
	return n;
}
function je(e) {
...
function Me(e, t, n, r, i) {
	...
	if (e && !t.ignoreAttributes) for (let o in e) {
		...
		n ? l = e[o] : (l = t.attributeValueProcessor(o, e[o]), l = Pe(l, t)), l === !0 && t.suppressBooleanAttributes ? a += ` ${c}` : a += ` ${c}="${ue(l)}"`;
	}
	return a;
}
function Ne(e, t) {
...
function Le(e) {
	if (this.options = Object.assign({}, Ie, e), this.options.stopNodes && Array.isArray(this.options.stopNodes) && (this.options.stopNodes = this.options.stopNodes.map((e) => typeof e == "string" && e.startsWith("*.") ? ".." + e.substring(2) : e)), this.stopNodeExpressions = [], this.options.stopNodes && Array.isArray(this.options.stopNodes)) for (let e = 0; e < this.options.stopNodes.length; e++) {
		...
		typeof t == "string" ? this.stopNodeExpressions.push(new M(t)) : t instanceof M && this.stopNodeExpressions.push(t);
	}
...
Le.prototype.build = function(e) {
	...
	for (let c in e) {
		...
		if (e[c] === void 0) this.isAttribute(c) && (a += "");
			...
			if (u && !this.ignoreAttributesFn(u, o)) {
				...
				i += this.buildAttrPairStr(t, N(e[c]), s);
			} else if (!u) {
				if (c === this.options.textNodeName) {
					let t = this.options.tagValueProcessor(c, N(e[c]));
			...
			for (let u = 0; u < i; u++) {
				...
				if (i !== void 0) {
					if (i === null) l[0] === "?" ? a += this.indentate(t) + "<" + l + "?" + this.tagEndChar : a += this.indentate(t) + "<" + l + "/" + this.tagEndChar;
						...
						e = this.replaceEntitiesValue(e), e = N(e), o += e;
					} else {
						n.push(l);
			...
			for (let o = 0; o < a; o++) {
				...
				i += this.buildAttrPairStr(a, N(e[c][t[o]]), s);
			}
	...
	return n || (t = this.options.attributeValueProcessor(e, N(t)), t = this.replaceEntitiesValue(t)), this.options.suppressBooleanAttributes && t === "true" ? " " + e : " " + e + "=\"" + ue(t) + "\"";
};
...
Le.prototype.extractAttributes = function(e) {
	...
	if (this.options.attributesGroupName && e[this.options.attributesGroupName]) {
		...
		for (let e in n) {
			...
			i === !0 && this.options.suppressBooleanAttributes ? t += " " + r : t += " " + r + "=\"" + ue(i) + "\"";
		}
	} else for (let n in e) {
	...
	if (t[0] === "?") return this.indentate(r) + "<" + t + n + "?" + this.tagEndChar;
	{
		...
		return i = this.replaceEntitiesValue(i), i = N(i), i === "" ? this.indentate(r) + "<" + t + n + this.closeTag(t) + this.tagEndChar : this.indentate(r) + "<" + t + n + ">" + i + "</" + t + this.tagEndChar;
	}
	...
	for (let a = 0; a < e.length; a++) if (e[a] === "<" && e[a + 1] === "?") {
		...
		}
		{
			...
			if (c = c.trim(), c[c.length - 1] === "/" && (c = c.substring(0, c.length - 1), a--), !ut(c)) {
				let t;
				return t = c.trim().length === 0 ? "Invalid space after '<'." : "Tag '" + c + "' is an invalid name.", F("InvalidTag", t, dt(e, a));
			}
			let l = it(e, a);
			if (l === !1) return F("InvalidAttr", "Attributes for '" + c + "' have open quote.", dt(e, a));
			let u = l.value;
			if (a = l.index, u[u.length - 1] === "/") {
				...
				let i = ot(u, t);
				if (i === !0) r = !0;
				else return F(i.err.code, i.err.msg, dt(e, n + i.err.line));
			} else if (s) {
				if (!l.tagClosed) return F("InvalidTag", "Closing tag '" + c + "' doesn't have proper closing.", dt(e, a));
				if (u.trim().length > 0) return F("InvalidTag", "Closing tag '" + c + "' can't have attributes or invalid starting.", dt(e, o));
				if (n.length === 0) return F("InvalidTag", "Closing tag '" + c + "' has not been opened.", dt(e, o));
				{
					let t = n.pop();
					if (c !== t.tagName) {
						let n = dt(e, t.tagStartPos);
						return F("InvalidTag", "Expected closing tag '" + t.tagName + "' (opened in line " + n.line + ", col " + n.col + ") instead of closing tag '" + c + "'.", dt(e, o));
					}
					n.length == 0 && (i = !0);
				}
			} else {
				let s = ot(u, t);
				if (s !== !0) return F(s.err.code, s.err.msg, dt(e, a - u.length + s.err.line));
				if (i === !0) return F("InvalidXml", "Multiple possible root nodes found.", dt(e, a));
			...
			for (a++; a < e.length; a++) if (e[a] === "<") {
				...
				let t = ct(e, a);
				if (t == -1) return F("InvalidChar", "char '&' is not expected.", dt(e, a));
				a = t;
			} else if (i === !0 && !$e(e[a])) return F("InvalidXml", "Extra text at the end", dt(e, a));
			e[a] === "<" && a--;
		}
	} else {
		if ($e(e[a])) continue;
		return F("InvalidChar", "char '" + e[a] + "' is not expected.", dt(e, a));
	}
	return r ? n.length == 1 ? F("InvalidTag", "Unclosed tag '" + n[0].tagName + "'.", dt(e, n[0].tagStartPos)) : n.length > 0 ? F("InvalidXml", "Invalid '" + JSON.stringify(n.map((e) => e.tagName), null, 4).replace(/\r?\n/g, "") + "' found.", {
		line: 1,
		col: 1
	}) : !0 : F("InvalidXml", "Start tag expected.", 1);
}
function $e(e) {
	return e === " " || e === "	" || e === "\n" || e === "\r";
}
function et(e, t) {
	...
	for (; t < e.length; t++) if (e[t] == "?" || e[t] == " ") {
		...
		if (t > 5 && r === "xml") return F("InvalidXml", "XML declaration allowed only at the start of the document.", dt(e, t));
		if (e[t] == "?" && e[t + 1] == ">") {
			t++;
			break;
		}
...
function ot(e, t) {
	...
	for (let e = 0; e < n.length; e++) {
		if (n[e][1].length === 0) return F("InvalidAttr", "Attribute '" + n[e][2] + "' has no space in starting.", ft(n[e]));
		if (n[e][3] !== void 0 && n[e][4] === void 0) return F("InvalidAttr", "Attribute '" + n[e][2] + "' is without value.", ft(n[e]));
		if (n[e][3] === void 0 && !t.allowBooleanAttributes) return F("InvalidAttr", "boolean attribute '" + n[e][2] + "' is not allowed.", ft(n[e]));
		let i = n[e][2];
		if (!lt(i)) return F("InvalidAttr", "Attribute '" + i + "' is an invalid name.", ft(n[e]));
		if (!Object.prototype.hasOwnProperty.call(r, i)) r[i] = 1;
		else return F("InvalidAttr", "Attribute '" + i + "' is repeated.", ft(n[e]));
	}
	return !0;
}
function st(e, t) {
...
function vt(e) {
	...
	for (let t of e) if (_t.has(t)) throw Error(`[EntityReplacer] Invalid character '${t}' in entity name: "${e}"`);
	return e;
}
function yt(...e) {
...
var St = "all";
function Ct(e) {
	return !e || e === bt ? /* @__PURE__ */ new Set([bt]) : e === St ? /* @__PURE__ */ new Set([St]) : e === xt ? /* @__PURE__ */ new Set([xt]) : Array.isArray(e) ? new Set(e) : /* @__PURE__ */ new Set([bt]);
}
...
function Et(e) {
	...
	let t = e.xmlVersion === 1.1 ? 1.1 : 1, n = wt[e.onNCR] ?? wt.allow, r = wt[e.nullNCR] ?? wt.remove;
	return {
		xmlVersion: t,
		onLevel: n,
		nullLevel: Math.max(r, wt.remove)
	};
}
var Dt = class {
	constructor(e = {}) {
		this._limit = e.limit || {}, this._maxTotalExpansions = this._limit.maxTotalExpansions || 0, this._maxExpandedLength = this._limit.maxExpandedLength || 0, this._postCheck = typeof e.postCheck == "function" ? e.postCheck : (e) => e, this._limitTiers = Ct(this._limit.applyLimitsTo ?? bt), this._numericAllowed = e.numericAllowed ?? !0, this._baseMap = yt(mt, e.namedEntities || null), this._externalMap = Object.create(null), this._inputMap = Object.create(null), this._totalExpansions = 0, this._expandedLength = 0, this._removeSet = new Set(e.remove && Array.isArray(e.remove) ? e.remove : []), this._leaveSet = new Set(e.leave && Array.isArray(e.leave) ? e.leave : []);
		let t = Et(e.ncr);
	...
	_applyRegistrationHook(e, t, n, r) {
		if (!e) return !0;
		let i = e(t, n);
		if (i === gt.BLOCK) return !1;
		if (i === gt.THROW) throw Error(`[EntityDecoder] Registration of ${r} entity "&${t};" was rejected by hook`);
		return !0;
	}
	setExternalEntities(e) {
		if (e) for (let t of Object.keys(e)) vt(t);
		if (!this._onExternalEntity) {
			this._externalMap = yt(e);
			return;
		}
		let t = yt(e), n = Object.create(null);
	...
	addExternalEntity(e, t) {
		vt(e), typeof t == "string" && t.indexOf("&") === -1 && this._applyRegistrationHook(this._onExternalEntity, e, t, "external") && (this._externalMap[e] = t);
	}
	addInputEntities(e) {
	...
	decode(e) {
		...
		for (; a < r;) {
			...
			let u, d;
			if (this._removeSet.has(l)) u = "", d === void 0 && (d = bt);
	...
	_tierCounts(e) {
		return this._limitTiers.has(St) ? !0 : this._limitTiers.has(e);
	}
	_resolveName(e) {
	...
	_classifyNCR(e) {
		return e === 0 ? this._ncrNullLevel : e >= 55296 && e <= 57343 || this._ncrXmlVersion === 1 && e >= 1 && e <= 31 && !Tt.has(e) ? wt.remove : -1;
	}
	_applyNCRAction(e, t, n) {
		switch (e) {
			case wt.allow: return String.fromCodePoint(n);
			case wt.remove: return "";
			case wt.leave: return;
			case wt.throw: throw Error(`[EntityDecoder] Prohibited numeric character reference &${t}; (U+${n.toString(16).toUpperCase().padStart(4, "0")})`);
...
var Mt = function(e) {
	...
	for (let { value: e, name: t } of n) e && At(e, t);
	return t.onDangerousProperty === null && (t.onDangerousProperty = Ot), t.processEntities = jt(t.processEntities, t.htmlEntities), t.unpairedTagsSet = new Set(t.unpairedTags), t.stopNodes && Array.isArray(t.stopNodes) && (t.stopNodes = t.stopNodes.map((e) => typeof e == "string" && e.startsWith("*.") ? ".." + e.substring(2) : e)), t;
};
var Nt = typeof Symbol == "function" ? Symbol("XML Node Metadata") : "@@xmlMetadata";
var Pt = class {
	constructor(e) {
	...
	addStartIndex(e) {
		e !== void 0 && (this.child[this.child.length - 1][Nt] = { startIndex: e });
	}
	addEndIndex(e) {
		let t = this.child[this.child.length - 1];
		t !== void 0 && t[Nt] !== void 0 && t[Nt].endIndex === void 0 && (t[Nt].endIndex = e);
	}
...
var Ft = class {
		...
		if (e[t + 3] === "O" && e[t + 4] === "C" && e[t + 5] === "T" && e[t + 6] === "Y" && e[t + 7] === "P" && e[t + 8] === "E") {
			...
			for (; t < e.length; t++) {
				...
				if (e[t] === "<" && !o) {
					...
					} else if (a && Lt(e, "!ELEMENT", t)) {
						t += 8;
						let { index: n } = this.readElementExp(e, t + 1);
						t = n;
					} else if (a && Lt(e, "!ATTLIST", t)) t += 8;
					else if (a && Lt(e, "!NOTATION", t)) {
						t += 9;
	...
	readEntityExp(e, t) {
		...
		if (Rt(r, { xmlVersion: this.xmlVersion }), t = It(e, t), !this.suppressValidationErr) {
	...
	readNotationExp(e, t) {
		...
		let r = e.substring(n, t);
		!this.suppressValidationErr && Rt(r, { xmlVersion: this.xmlVersion }), t = It(e, t);
		...
		t += i.length, t = It(e, t);
		let a = null, o = null;
		if (i === "PUBLIC") [t, a] = this.readIdentifierVal(e, t, "publicIdentifier"), t = It(e, t), (e[t] === "\"" || e[t] === "'") && ([t, o] = this.readIdentifierVal(e, t, "systemIdentifier"));
	...
	readElementExp(e, t) {
		...
		t = It(e, t);
		let i = "";
		if (e[t] === "E" && Lt(e, "MPTY", t)) t += 4;
		else if (e[t] === "A" && Lt(e, "NY", t)) t += 2;
		else if (e[t] === "(") {
			t++;
			let n = t;
	...
	readAttlistExp(e, t) {
		...
		for (; t < e.length && !/\s/.test(e[t]);) t++;
		let r = e.substring(n, t);
		for (Rt(r, { xmlVersion: this.xmlVersion }), t = It(e, t), n = t; t < e.length && !/\s/.test(e[t]);) t++;
		let i = e.substring(n, t);
		if (!Rt(i, { xmlVersion: this.xmlVersion })) throw Error(`Invalid attribute name: "${i}"`);
		t = It(e, t);
		let a = "";
		if (e.substring(t, t + 8).toUpperCase() === "NOTATION") {
			if (a = "NOTATION", t += 8, t = It(e, t), e[t] !== "(") throw Error(`Expected '(', found "${e[t]}"`);
			t++;
			let n = [];
			for (; t < e.length && e[t] !== ")";) {
				let r = t;
				...
				if (i = i.trim(), !Rt(i, { xmlVersion: this.xmlVersion })) throw Error(`Invalid notation name: "${i}"`);
				n.push(i), e[t] === "|" && (t++, t = It(e, t));
			}
...
var Vt = 65535;
var Ht = 1632;
var Ut = (/* @__PURE__ */ new Uint8Array(63904)).fill(255);
for (let e of zt) for (let t = 0; t < 10; t++) {
	let n = e + t;
	n <= Vt ? Ut[n - Ht] = t : Bt.set(n, t);
}
var Wt = 48;
var Gt = 57;
var Kt = 45;
	...
	for (let i = n; i < t; i++) {
		...
		if (n >= 55296 && n <= 56319) {
			if (i + 1 < t) {
				...
				if (t >= 56320 && t <= 57343) {
					let e = 65536 + (n - 55296 << 10) + (t - 56320), a = Bt.get(e);
					if (a !== void 0) {
...
function en(e, t = {}) {
	if (t = Object.assign({}, $t, t), !e || typeof e != "string") return e;
	let n = e.trim();
	if (n.length === 0 || t.skipLike !== void 0 && t.skipLike.test(n)) return e;
	if (n === "0" || t.unicode && (n = Jt(n), n === "0")) return 0;
	if (t.hex && Yt.test(n)) return an(n, 16);
	if (t.binary && Xt.test(n)) return an(n, 2);
	if (t.octal && Zt.test(n)) return an(n, 8);
	if (!isFinite(n)) return on(e, Number(n), t);
	if (n.includes("e") || n.includes("E")) return nn(e, n, t);
	{
		let r = Qt.exec(n);
		if (r) {
			let i = r[1] || "", a = r[2], o = rn(r[3]), s = i ? e[a.length + 1] === "." : e[a.length] === ".";
...
var hn = [
	{
		...
		pattern: RegExp(`\\$where${mn}`, "i")
	},
	{
		id: "nosql-ne-operator",
		description: "$ne — \"not equal\" operator used to bypass equality checks",
		pattern: RegExp(`\\$ne${mn}`, "i")
	},
	{
		id: "nosql-gt-operator",
		description: "$gt — \"greater than\" used to bypass password/value checks",
		pattern: RegExp(`\\$gte?${mn}`, "i")
	},
	{
		id: "nosql-lt-operator",
		description: "$lt / $lte — \"less than\" bypass variants",
		pattern: RegExp(`\\$lte?${mn}`, "i")
	},
	{
		id: "nosql-regex-operator",
		description: "$regex — can be used to extract data character by character (blind injection)",
		pattern: RegExp(`\\$regex${mn}`, "i")
	},
	{
		id: "nosql-or-operator",
		description: "$or — logical OR; used to create always-true conditions",
		pattern: RegExp(`\\$or${mn}\\s*\\[`, "i")
	},
	{
		id: "nosql-and-operator",
		description: "$and — logical AND operator injection",
		pattern: RegExp(`\\$and${mn}\\s*\\[`, "i")
	},
	{
		id: "nosql-nor-operator",
		description: "$nor — logical NOR operator injection",
		pattern: RegExp(`\\$nor${mn}\\s*\\[`, "i")
	},
	{
		id: "nosql-exists-operator",
		description: "$exists — can enumerate fields to determine schema",
		pattern: RegExp(`\\$exists${mn}`, "i")
	},
	{
		id: "nosql-in-operator",
		description: "$in — matches any value in a list; can enumerate values",
		pattern: RegExp(`\\$in${mn}\\s*\\[`, "i")
	},
	{
		id: "nosql-expr-operator",
		description: "$expr — allows aggregation expressions in queries (MongoDB 3.6+)",
		pattern: RegExp(`\\$expr${mn}`, "i")
	},
	{
		id: "nosql-function-operator",
		description: "$function — executes arbitrary JavaScript in MongoDB 4.4+",
		pattern: RegExp(`\\$function${mn}`, "i")
	},
	{
		id: "nosql-accumulator-operator",
		description: "$accumulator — custom aggregation with arbitrary JS execution",
		pattern: RegExp(`\\$accumulator${mn}`, "i")
	},
	{
...
var vn = [...dn, ..._n];
cn.label = "HTML", ln.label = "XML", un.label = "SVG", dn.label = "SQL", vn.label = "SQL-STRICT", fn.label = "SHELL", pn.label = "REDOS", hn.label = "NOSQL", gn.label = "LOG", Object.freeze({
	HTML: cn,
	XML: ln,
	SVG: un,
	SQL: dn,
...
function Cn(e, t) {
	yn(e), bn(t);
	let { lists: n, regex: r } = xn(t);
	if (r) return r.test(e);
	for (let t of n) if (Sn(e, t) !== null) return !0;
	return !1;
}
function wn(e, t) {
	if (!e) return {};
...
var En = class {
	constructor(e, t) {
		this.options = e, this.currentNode = null, this.tagsNodeStack = [], this.parseXml = jn, this.parseTextData = Dn, this.resolveNameSpace = On, this.buildAttributesMap = An, this.isItStopNode = Fn, this.replaceEntitiesValue = Nn, this.readStopNodeData = Bn, this.saveTextToParentTag = Pn, this.addChild = Mn, this.ignoreAttributesFn = sn(this.options.ignoreAttributes), this.entityExpansionCount = 0, this.currentExpandedLength = 0, this.doctypefound = !1;
		let n = { ...mt };
		this.options.entityDecoder ? this.entityDecoder = this.options.entityDecoder : (typeof this.options.htmlEntities == "object" ? n = this.options.htmlEntities : this.options.htmlEntities === !0 && (n = {
			...
			onInputEntity: (e, t) => Cn(t, [cn, ln]) ? gt.BLOCK : gt.ALLOW
		})), this.matcher = new ce(), this.readonlyMatcher = this.matcher.readOnly(), this.isCurrentNodeStopNode = !1, this.stopNodeExpressionsSet = new oe();
		let r = this.options.stopNodes;
		if (r && r.length > 0) {
			for (let e = 0; e < r.length; e++) {
				let t = r[e];
				typeof t == "string" ? this.stopNodeExpressionsSet.add(new M(t)) : t instanceof M && this.stopNodeExpressionsSet.add(t);
			}
...
function Dn(e, t, n, r, i, a, o) {
	...
	if (e !== void 0 && (s.trimValues && !r && (e = e.trim()), e.length > 0)) {
		...
		return c == null ? e : typeof c != typeof e || c !== e ? c : s.trimValues || e.trim() === e ? Vn(e, s.parseTagValue, s.numberParseOptions) : e;
	}
}
function On(e) {
...
var kn = /* @__PURE__ */ RegExp("([^\\s=]+)\\s*(=\\s*(['\"])([\\s\\S]*?)\\3)?", "gm");
function An(e, t, n, r = !1) {
	let i = this.options;
	if (r === !0 || i.ignoreAttributes !== !0 && typeof e == "string") {
		let r = Ke(e, kn), a = r.length, o = {}, s = Array(a), c = !1, l = {};
		for (let e = 0; e < a; e++) {
		...
		for (let e = 0; e < a; e++) {
			...
			if (t.length) {
				if (i.transformAttributeName && (n = i.transformAttributeName(n)), n = Un(n, i), r[e][4] !== void 0) {
					let r = s[e], a = i.attributeValueProcessor(t, r, u);
					a == null ? o[n] = r : typeof a != typeof r || a !== r ? o[n] = a : o[n] = Vn(r, i.parseAttributeValue, i.numberParseOptions), d = !0;
...
var jn = function(e) {
	e = e.replace(/\r\n?/g, "\n");
	let t = new Pt("!xml"), n = t, r = "";
	this.matcher.reset(), this.entityDecoder.reset(), this.entityExpansionCount = 0, this.currentExpandedLength = 0, this.doctypefound = !1;
	let i = this.options, a = new Ft(i.processEntities), o = e.length;
	for (let s = 0; s < o; s++) if (e[s] === "<") {
		let c = e.charCodeAt(s + 1);
		if (c === 47) {
			let a = Ln(e, ">", s, "Closing Tag is not closed."), o = e.substring(s + 2, a).trim();
			if (i.removeNSPrefix) {
				let e = o.indexOf(":");
				e !== -1 && (o = o.substr(e + 1));
			}
			o = Hn(i.transformTagName, o, "", i).tagName, n && (r = this.saveTextToParentTag(r, n, this.readonlyMatcher));
			...
			let t = Ln(e, "-->", s + 4, "Comment is not closed.");
			...
			let t = Ln(e, "]]>", s, "CDATA is not closed.") - 2, a = e.substring(s + 9, t);
			r = this.saveTextToParentTag(r, n, this.readonlyMatcher);
			...
			if ({tagName: c, tagExp: u} = Hn(i.transformTagName, c, u, i), i.strictReservedNames && (c === i.commentPropName || c === i.cdataPropName || c === i.textNodeName || c === i.attributesGroupName)) throw Error(`Invalid tag name: ${c}`);
			...
			let h = null, g;
			g = Tn(l), c !== t.tagname && this.matcher.push(c, {}, g), c !== u && d && (h = this.buildAttributesMap(u, this.matcher, c), h && wn(h, i)), c !== t.tagname && (this.isCurrentNodeStopNode = this.isItStopNode());
			let _ = s;
			if (this.isCurrentNodeStopNode) {
				...
				if (m) {
					({tagName: c, tagExp: u} = Hn(i.transformTagName, c, u, i));
					let e = new Pt(c);
...
function Bn(e, t, n) {
	...
	for (; n < a; n++) if (e[n] === "<") {
		...
		if (a === 47) {
			let a = Rn(e, ">", n, `${t} is not closed`);
			if (e.substring(n + 2, a).trim() === t && (i--, i === 0)) return {
				tagContent: e.substring(r, n),
				i: a
			};
			n = a;
		} else if (a === 63) n = Ln(e, "?>", n + 1, "StopNode is not closed.");
		else if (a === 33 && e.charCodeAt(n + 2) === 45 && e.charCodeAt(n + 3) === 45) n = Ln(e, "-->", n + 3, "StopNode is not closed.");
		else if (a === 33 && e.charCodeAt(n + 2) === 91) n = Ln(e, "]]>", n, "StopNode is not closed.") - 2;
		else {
			let r = zn(e, n, !1);
...
function qn(e, t, n, r) {
	...
	for (let o = 0; o < e.length; o++) {
		...
		if (c !== void 0 && c !== t.textNodeName) {
			let e = Gn(s[":@"] || {}, t.attributeNamePrefix);
			n.push(c, e);
		}
		if (c === t.textNodeName) i === void 0 ? i = s[c] : i += "" + s[c];
		else if (c === void 0) continue;
		else if (s[c]) {
			let e = qn(s[c], t, n, r), i = Xn(e, t);
			if (Object.keys(e).length === 0 && t.alwaysCreateTextNode && (e[t.textNodeName] = ""), s[":@"] ? Yn(e, s[":@"], r, t) : Object.keys(e).length === 1 && e[t.textNodeName] !== void 0 && !t.alwaysCreateTextNode ? e = e[t.textNodeName] : Object.keys(e).length === 0 && (t.alwaysCreateTextNode ? e[t.textNodeName] = "" : e = ""), s[Wn] !== void 0 && typeof e == "object" && e && (e[Wn] = s[Wn]), a[c] !== void 0 && Object.prototype.hasOwnProperty.call(a, c)) Array.isArray(a[c]) || (a[c] = [a[c]]), a[c].push(e);
			else {
...
var Zn = class {
	...
	parse(e, t) {
		...
		let n = new En(this.options, this.externalEntities), r = n.parseXml(e);
		return this.options.preserveOrder || r === void 0 ? r : Kn(r, this.options, n.matcher, n.readonlyMatcher);
	}
	addEntity(e, t) {
...
var Qn = Symbol.for("__MUTATIVE_PROXY_DRAFT__");
var $n = Symbol("__MUTATIVE_RAW_RETURN_SYMBOL__");
...
function L(e) {
	return typeof e == "object" ? e?.[Qn] : null;
}
function lr(e) {
	let t = L(e);
...
function ur(e, t) {
	...
	let n;
	return Object.getPrototypeOf(e) === Object.prototype || Array.isArray(e) || e instanceof Map || e instanceof Set || !!t?.mark && ((n = t.mark(e, tr)) === tr.immutable || typeof n == "function");
}
function dr(e, t = []) {
	if (Object.hasOwnProperty.call(e, "key")) {
		let n = e.parent.copy, r = L(pr(n, e.key));
		if (r !== null && r?.original !== e.original) return null;
		let i = e.parent.type === 3, a = i ? Array.from(e.parent.setMap.keys()).indexOf(e.key) : e.key;
		if (!(i && n.size > a || rr(n, a))) return null;
		t.push(a);
	}
	if (e.parent) return dr(e.parent, t);
	t.reverse();
	try {
		yr(e.copy, t);
	} catch {
...
function yr(e, t) {
	for (let n = 0; n < t.length - 1; n += 1) {
		let r = t[n];
		if (e = pr(fr(e) === 3 ? Array.from(e) : e, r), typeof e != "object") throw Error(`Cannot resolve patch at '${t.join("/")}'.`);
	}
	return e;
}
function br(e) {
...
function Sr(e, t) {
	...
	if (t?.mark && (n = t.mark(e, tr), n !== void 0) && n !== tr.mutable) {
		if (n === tr.immutable) return br(e);
...
function Cr(e) {
	e.copy ||= Sr(e.original, e.options);
}
function wr(e) {
	if (!ur(e)) return lr(e);
	if (Array.isArray(e)) return e.map(wr);
	if (e instanceof Map) {
		let t = Array.from(e.entries()).map(([e, t]) => [e, wr(t)]);
		if (!or(e)) {
...
function Er(e) {
	e.assignedMap = e.assignedMap ?? /* @__PURE__ */ new Map(), e.operated || (e.operated = !0, e.parent && Er(e.parent));
}
function Dr() {
...
function Or(e, t, n, r, i) {
	...
	if (Object.isFrozen(e) || cr(e)) {
		r.pop(), i.pop();
		return;
	}
	switch (fr(e)) {
		case 2:
			for (let [t, a] of e) Or(t, t, n, r, i), Or(a, t, n, r, i);
			e.set = e.clear = e.delete = Dr;
			break;
		case 3:
			for (let t of e) Or(t, t, n, r, i);
			e.add = e.clear = e.delete = Dr;
			break;
		case 1:
			Object.freeze(e);
			let t = 0;
			for (let a of e) Or(a, t, n, r, i), t += 1;
			break;
...
function Ar(e, t, n) {
	if (cr(e) || !ur(e, n) || t.has(e) || Object.isFrozen(e)) return;
...
function jr(e, t) {
	...
	e.finalities.revoke.length > 1 && e.assignedMap.get(t) && n && Ar(pr(n, t), e.finalities.handledSet, e.options);
}
function Mr(e) {
...
function Pr(e, t, n, r) {
	...
	i && (i.callbacks ||= [], i.callbacks.push((a, o) => {
		...
		if (gr(pr(s, t), n)) {
			let n = i.original;
			i.copy && (n = i.copy), Mr(e), Nr(e, r, a, o), e.options.enableAutoFreeze && (e.options.updatedValues = e.options.updatedValues ?? /* @__PURE__ */ new WeakMap(), e.options.updatedValues.set(n, i.original)), mr(s, t, n);
		}
	}), e.options.enableAutoFreeze && i.finalities !== e.finalities && (e.options.enableAutoFreeze = !1)), ur(n, e.options) && e.finalities.draft.push(() => {
		gr(pr(e.type === 3 ? e.setMap : e.copy, t), n) && jr(e, t);
	});
}
...
function Ir({ original: e, copy: t, assignedMap: n }, r, i, a, o) {
	n.forEach((n, s) => {
		let c = pr(e, s), l = Tr(pr(t, s)), u = n ? rr(e, s) ? I.Replace : I.Add : I.Remove;
		if (gr(c, l) && u === I.Replace) return;
		let d = vr(r.concat(s), o);
		i.push(u === I.Remove ? {
			op: u,
			path: d
		} : {
...
function Lr({ original: e, copy: t }, n, r, i, a) {
	let o = 0;
	e.forEach((e) => {
		if (!t.has(e)) {
			let t = vr(n.concat([o]), a);
			r.push({
				op: I.Remove,
				path: t,
				value: e
...
function Rr(e, t, n, r) {
	...
	switch (e.type) {
		case 0:
		case 2: return Ir(e, t, n, r, i);
		case 1: return Fr(e, t, n, r, i);
		case 3: return Lr(e, t, n, r, i);
	}
}
var zr = (e, t, n = !1) => {
	if (typeof e == "object" && e && (!ur(e, t) || n)) throw Error("Strict mode: Mutable data cannot be accessed directly, please use 'unsafe(callback)' wrap.");
};
var Br = {
	get size() {
		return sr(L(this)).size;
	},
	has(e) {
		return sr(L(this)).has(e);
	},
	set(e, t) {
		let n = L(this), r = sr(n);
		return (!r.has(e) || !gr(r.get(e), t)) && (Cr(n), Er(n), n.assignedMap.set(e, !0), n.copy.set(e, t), Pr(n, e, t, Rr)), this;
	},
	delete(e) {
		if (!this.has(e)) return !1;
		let t = L(this);
		return Cr(t), Er(t), t.original.has(e) ? t.assignedMap.set(e, !1) : t.assignedMap.delete(e), t.copy.delete(e), !0;
	},
	clear() {
		let e = L(this);
		if (this.size) {
			Cr(e), Er(e), e.assignedMap = /* @__PURE__ */ new Map();
			for (let [t] of e.original) e.assignedMap.set(t, !1);
			e.copy.clear();
		}
	},
	forEach(e, t) {
		sr(L(this)).forEach((n, r) => {
			e.call(t, this.get(r), r, this);
		});
	},
	get(e) {
		var t;
		let n = L(this), r = sr(n).get(e), i = (t = n.options).mark?.call(t, r, tr) === tr.mutable;
		if (n.options.strict && zr(r, n.options, i), i || n.finalized || !ur(r, n.options) || r !== n.original.get(e)) return r;
		let a = nr.createDraft({
			original: r,
			parentDraft: n,
			key: e,
		...
		return Cr(n), n.copy.set(e, a), a;
	},
	keys() {
		return sr(L(this)).keys();
	},
	values() {
		let e = this.keys();
		return {
...
var Vr = Reflect.ownKeys(Br);
var Hr = (e, t, { isValuesIterator: n }) => () => {
	var r;
	let i = t.next();
	if (i.done) return i;
	let a = i.value, o = e.setMap.get(a), s = L(o), c = (r = e.options).mark?.call(r, o, tr) === tr.mutable;
	if (e.options.strict && zr(a, e.options, c), !c && !s && ur(a, e.options) && !e.finalized && e.original.has(a)) {
		let t = nr.createDraft({
			original: a,
			parentDraft: e,
			key: a,
...
var Ur = {
	...
	add(e) {
		let t = L(this);
		return this.has(e) || (Cr(t), Er(t), t.assignedMap.set(e, !0), t.setMap.set(e, e), Pr(t, e, e, Rr)), this;
	},
	delete(e) {
		if (!this.has(e)) return !1;
		let t = L(this);
	...
	values() {
		...
		return {
			...
			next: Hr(e, t, { isValuesIterator: !0 })
		};
	},
	entries() {
		let e = L(this);
		...
		return {
			...
			next: Hr(e, t, { isValuesIterator: !1 })
		};
	},
	keys() {
		return this.values();
...
Set.prototype.difference && Object.assign(Ur, {
	intersection(e) {
...
var Gr = {
	get(e, t, n) {
		...
		if (e.options.mark) {
			...
			if (i = e.options.mark(r, tr), i === tr.mutable) return e.options.strict && zr(r, e.options, !0), r;
		}
		let a = sr(e);
		if (a instanceof Map && Vr.includes(t)) return t === "size" ? Object.getOwnPropertyDescriptor(Br, "size").get.call(e.proxy) : Br[t].bind(e.proxy);
		if (a instanceof Set && Wr.includes(t)) return t === "size" ? Object.getOwnPropertyDescriptor(Ur, "size").get.call(e.proxy) : Ur[t].bind(e.proxy);
		if (!rr(a, t)) {
			let n = ir(a, t);
			return n ? "value" in n ? n.value : n.get?.call(e.proxy) : void 0;
		}
		let o = a[t];
		if (e.options.strict && zr(o, e.options), e.finalized || !ur(o, e.options)) return o;
		if (o === hr(e.original, t)) {
			if (Cr(e), e.copy[t] = Kr({
				...
				let n = L(e.copy[t]);
				return Cr(n), Er(n), n.copy;
			}
			return e.copy[t];
		}
		return cr(o) && e.finalities.draftsCache.add(o), o;
	},
	set(e, t, n) {
		...
		let a = hr(sr(e), t), o = L(a);
		return o && gr(o.original, n) ? (e.copy[t] = n, e.assignedMap = e.assignedMap ?? /* @__PURE__ */ new Map(), e.assignedMap.set(t, !1), !0) : gr(n, a) && (n !== void 0 || rr(e.original, t)) ? !0 : (Cr(e), Er(e), rr(e.original, t) && gr(n, e.original[t]) ? e.assignedMap.delete(t) : e.assignedMap.set(t, !0), e.copy[t] = n, Pr(e, t, n, Rr), !0);
	},
	has(e, t) {
		return t in sr(e);
	},
	...
	getOwnPropertyDescriptor(e, t) {
		let n = sr(e), r = Reflect.getOwnPropertyDescriptor(n, t);
		return r && {
			writable: !0,
	...
	deleteProperty(e, t) {
		return e.type === 1 ? Gr.set.call(this, e, t, void 0, e.proxy) : (hr(e.original, t) !== void 0 || t in e.original ? (Cr(e), Er(e), e.assignedMap.set(t, !1)) : (e.assignedMap = e.assignedMap ?? /* @__PURE__ */ new Map(), e.assignedMap.delete(t)), e.copy && delete e.copy[t], !0);
	}
};
function Kr(e) {
	let { original: t, parentDraft: n, key: r, finalities: i, options: a } = e, o = fr(t), s = {
		type: o,
		finalized: !1,
		parent: n,
		original: t,
	...
	(r || "key" in e) && (s.key = r);
	let { proxy: c, revoke: l } = Proxy.revocable(o === 1 ? Object.assign([], s) : s, Gr);
	if (i.revoke.push(l), s.proxy = c, n) {
		let e = n;
		e.finalities.draft.push((t, n) => {
			var i;
			let a = L(c), o = e.type === 3 ? e.setMap : e.copy, s = pr(o, r), l = L(s);
			if (l) {
				let i = l.original;
				l.operated && (i = lr(s)), Mr(l), Nr(l, Rr, t, n), e.options.enableAutoFreeze && (e.options.updatedValues = e.options.updatedValues ?? /* @__PURE__ */ new WeakMap(), e.options.updatedValues.set(i, l.original)), mr(o, r, i);
			}
...
function Jr(e, t) {
	...
	t.enablePatches && (r = [], i = []);
	let a = t.mark?.call(t, e, tr) === tr.mutable || !ur(e, t) ? e : Kr({
		original: e,
		parentDraft: null,
		finalities: n,
		options: t
	...
	return [a, (e = []) => {
		let [n, o, s] = qr(a, e, r, i, t.enableAutoFreeze);
...
function Yr(e) {
	...
	kr(n, (n, r, i) => {
		...
		} else typeof r == "object" && r && (e.value = r, e.isRoot = !1, Yr(e));
...
function Xr(e) {
	let t = L(e);
	if (!ur(e, t?.options)) return e;
	let n = fr(e);
	if (t && !t.operated) return t.original;
	let r;
	function i() {
		r = n === 2 ? or(e) ? new Map(e) : new (Object.getPrototypeOf(e)).constructor(e) : n === 3 ? Array.from(t.setMap.values()) : Sr(e, t?.options);
	}
	if (t) {
		t.finalized = !0;
		try {
	...
	if (kr(r, (n, a) => {
		if (t && gr(pr(t.original, n), a)) return;
		let o = Xr(a);
		o !== a && (r === e && i(), mr(r, n, o));
	}), n === 3) {
		let e = t?.original ?? r;
		return ar(e) ? new Set(r) : new (Object.getPrototypeOf(e)).constructor(r);
	}
	return r;
}
function Zr(e) {
	if (!cr(e)) throw Error(`current() is only used for Draft, parameter: ${e}`);
	return Xr(e);
}
var Qr = ((e) => {
	...
	return function t(n, r, i) {
		...
		let c = cr(a) ? Zr(a) : a, l = Array.isArray(s.mark) ? ((e, t) => {
			for (let n of s.mark) {
		...
		if (!ur(c, f) && typeof c == "object" && c) throw Error("Invalid base state: create() only supports plain objects, arrays, Set, Map or using mark() to mark the state as immutable.");
		let [p, m] = Jr(c, f);
		if (typeof r != "function") {
			if (!ur(c, f)) throw Error("Invalid base state: create() only supports plain objects, arrays, Set, Map or using mark() to mark the state as immutable.");
			return [p, m];
		}
		let h;
		try {
		...
		let g = (e) => {
			let t = L(p);
			if (!cr(e)) {
				if (e !== void 0 && !gr(e, p) && t?.operated) throw Error("Either the value is returned as a new non-draft value, or only the draft is modified without returning any value.");
				let n = e?.[$n];
				if (n) {
					let r = n[0];
					return f.strict && typeof e == "object" && e && Yr({
						rootDraft: t,
						value: e,
						useRawReturn: !0
					}), m([r]);
				}
				if (e !== void 0) return typeof e == "object" && e && Yr({
					rootDraft: t,
					value: e
				}), m([e]);
			}
...
Object.keys(ai).reduce((e, t) => (e[ai[t]] = t, e), {});
var oi = {
	schema: {
		id: ii,
		name: ii,
...
function si(e) {
	oi.schema = W.toSchemaModelTreeWithJsObject({
		name: ii,
		disabled: !0,
		type: D.Object,
		children: e.children
	}), oi.nameReg = `^(${e.children.map((e) => e.name).join("|").replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})\\.`;
}
var ci = {
	id: "_system_",
	name: "_system_",
	label: A.systemVariables,
	type: D.Object,
	disabled: !0,
	children: [
		{
			id: ri,
			name: ri,
			tips: A.envVariableTips,
			type: D.String
		},
		{
			id: "''",
			name: "emptyString",
			tips: A.emptyStringTips,
			type: D.String
		},
		{
			id: "null",
...
var li = {
	isContainer(e) {
		return e === D.Object || e === D.Array || e === D.Map;
	},
	matchVariable(e) {
		if (e) {
	...
	getValidContextRef({ text: e, source: t }) {
		if (t === k.Variable) {
			let { fun: t, variable: n } = li.matchVariable(e);
			return t === H._number ? "0" : n;
		}
		return /\D/.test(e) ? "" : "0";
	},
	getVariableMode(e) {
		return e === "_key_" || e.startsWith("_item_") ? "context" : ci.children.some((t) => t.id === e) ? "constant" : e.startsWith("_utils") ? "utils" : "output";
	},
	getConstants() {
		return ci;
	},
					...
					children: [
						{
							id: B,
							name: B,
							type: t.type === D.Array ? D.Number : D.String
						},
						{
							id: V,
							name: V,
...
function mi(e, t, n, r, i, a, o) {
	return e.map((e) => {
		let s = j((n === D.Array || n === D.Map) && e.name === "*" && (a || o) ? "0" : e.name, e.optional, t, r), c = r ? s.replace(/\?$/, "") : s, l = {
			...e,
			id: c
		};
		i !== void 0 && (l.folded = i);
		let u = a && e.type === D.File ? [{
			name: "fileName",
			type: D.String
		}, {
			name: "fileSize",
		...
		return l.children = u ? mi(a && e.type === D.Array ? [{
			name: "length",
			type: D.Number
		}, ...u] : u, s, e.type, r, i, a, o) : void 0, l;
	});
}
function hi(e, t = "", n = !1, r, i, a) {
	let o = j(e.name, e.optional, t, n), s = i && e.type === D.File ? [{
		name: "fileName",
		type: D.String
	}, {
		name: "fileSize",
	...
	return {
		...
		id: o,
		folded: r,
		children: s ? mi(i && e.type === D.Array ? [{
			name: "length",
			type: D.Number
...
function gi(e, t, n) {
	...
	let r = typeof e, i = {
		name: t,
		type: pi[r === "object" ? Array.isArray(e) ? "array" : "*" in e ? "map" : "object" : r]
	};
	if (n.push(i), i.type === D.Object) {
		i.children = [];
		for (let t in e) {
			let n = e[t];
			gi(n, t, i.children);
		}
	} else i.type === D.Array ? (i.children = [], gi(e[0], "*", i.children)) : i.type === D.Map && (i.children = [], gi(e["*"], "*", i.children));
}
function _i(e) {
	switch (e) {
		case D.String: return "xxx";
		case D.Number: return 123;
		case D.Bool: return !0;
		case D.Date: return "1970-01-01 00:00:00";
		case D.Object: return {};
		case D.Array: return [];
		case D.Map: return {};
		case D.Any: return "xxx";
	}
}
function vi(e, t, n) {
	e.forEach((e) => {
		let r = t === D.Array ? "0" : e.name, i = _i(e.type);
		n[r] = i, e.children && vi(e.children, e.type, i);
	});
}
function yi(e, t) {
	let n = {};
	return e.map((e) => {
		...
		let o = t === D.Array || t === D.Map ? "*" : a.name, s = a.label;
		if (!o) throw A.nameIsRequired;
		if (n[o]) throw re(A.nameIsRepeat, { name: o });
		if (!D[r]) throw re(A.typeRestricted, { type: r });
		n[o] = !0;
		let c = e[r], l = {
			name: o,
			type: D[r]
		};
		return a.optional === "true" && (l.optional = !0), a.tips && (l.tips = a.tips), a.enums && (l.enums = JSON.parse(a.enums)), s && (l.label = s), l.type === D.Array || l.type === D.Map ? l.children = yi([c[0] || { [O[D.String]]: [] }], l.type) : l.type === D.Object && c.length && (l.children = yi(c, l.type)), l;
	});
}
var W = {
	toSchemaTitle(e, t, n) {
	...
	regenSchemaModelWithJsObject(e, t, n) {
		return hi(e, t, !0, n, !1, !0);
	},
	patchSchemaDirect(e, t, n, r) {
		if (e.direct) {
			let i = t[e.direct];
			if (i) {
				if ((i.type === D.Array || i.type === D.Map) && (e.name === "_item_" || e.name === "_key_")) {
					if (e.name === "_item_") {
						let i = t[`${e.direct}[0]`];
						if (i) {
						...
						Object.assign(e, {
							type: i.type === D.Array ? D.Number : D.String,
							direct: void 0
						});
						return;
					}
	...
	filterArraySpecific(e) {
		if (e.children) {
			let t = { ...e };
			return e.type === D.Array && (t.children = e.children.filter((e) => e.name === "*")), t.children = t.children.map((e) => W.filterArraySpecific(e)), t;
		}
		return e;
	},
	schemaModelToXmlData: fi,
	schemaModelToXml(e) {
		return ui.build([fi(e)]).trim();
	},
	xmlDataToSchemaModel: yi,
	xmlToSchemaModel(e) {
		return yi(di.parse(e))[0];
	},
	formatXml(e) {
		let t = di.parse(e);
		return ui.build(t).trim();
	},
	jsonToSchemaModel(e) {
		let t = JSON.parse(e);
		if (t == null) return {
		...
		let n = typeof t, r = {
			name: "???",
			type: pi[n === "object" ? Array.isArray(t) ? "array" : "*" in t ? "map" : "object" : n]
		};
		if (r.type === D.Object) {
			r.children = [];
			for (let e in t) {
		...
		} else r.type === D.Array ? (r.children = [], gi(t[0], "*", r.children)) : r.type === D.Map && (r.children = [], gi(t["*"], "*", r.children));
		return r;
	},
	schemaModelToJson(e) {
		let t = _i(e.type);
		return e.children && vi(e.children, e.type, t), JSON.stringify(t, null, 4);
	}
};
function bi(e, t) {
	return e.map((e) => {
		let n = j(e.name, !1, t, !1), r = {
			...e,
			id: n
		};
		return r.children = e.children ? bi(e.children, n) : void 0, r;
	});
}
function xi(e, t, n) {
	return e.map((e) => Si(e, t, n));
}
function Si(e, t, n) {
	let { id: r, name: i, type: a, optional: o } = e, s = e.children || [], c = typeof n == "string" ? j(i, !1, n, !1) : void 0, l = t[i], u = l?.value, d = l?.children, f = d?.reduce((e, t) => (e[t.name] = t, e), {}), p = s.length && d, m = u ? u.type === a && u.optional === o ? u : {
		...u,
		type: a,
		optional: o
	} : {
	...
	if (!p && m.text === "*" && (m.text = ""), p) {
		...
		if (a === D.Array || a === D.Map) {
			if (m.text === "*") {
				...
				} else a === D.Map && (s = Object.keys(f).map((i) => ({
					name: i,
					id: e,
					type: t,
					optional: n,
...
var G = {
	getValueMode(e) {
		...
		return n === D.Object ? t.text === "*" ? "deconstruct" : "assign" : n === D.Array || n === D.Map ? t.text === "*" ? "deconstruct" : e.children ? "mapping" : "assign" : "assign";
	},
	valueModeIsMapping(e) {
		let t = e.value, n = e.value.type;
		return !!((n === D.Array || n === D.Map) && t.text !== "*" && e.children);
	},
	toSchemaValueTree(e, t) {
		return bi(e, t);
	},
	createSchemaValueByModel(e, t, n) {
		return Si(n ? e : W.toSchemaModelTree(e), { [e.name]: t });
	},
	createSchemaValueTreeByModel(e, t, n = "", r) {
		return Si(r ? e : W.toSchemaModelTree(e), { [e.name]: t }, n);
	},
	checkForm(e) {
		return e.querySelector(".ͼbaseflow-SuperInput[data-error]")?.getAttribute("data-error") || "";
	},
	matchSchemaValueByModel(e, t) {
		if (e.name !== t.name && e.name !== "*") return `(${t.name})${A.nameMismatch}`;
		if (e.type !== t.value.type) return `(${t.name})${A.typeMismatch}`;
		if (e.optional !== t.value.optional) return `(${t.name})${A.optionalMismatch}`;
		let n = e.children, r = t.children;
		if (!n && r || n && !r && t.value.text === "*") return `(${t.name})${A.structureMismatch}`;
		if (n && r) {
			if (e.type === D.Object) {
				if (r.length !== n.length) return `(${t.name})${A.structureMismatch}`;
				...
				for (let t = 0, r = n.length; t < r; t++) {
					...
					if (i) {
						...
						if (e) return e;
					} else return `(${r.name})${A.requiredPrompt}`;
				}
			} else {
				let e = n[0];
	...
	setValueMode(e, t, n) {
		...
		let i = t.type, a = {
			type: i,
			optional: t.optional,
			source: n === "deconstruct" ? k.Template : k.Variable,
			text: n === "deconstruct" ? "*" : ""
		};
		if (n === "assign") return {
			...e,
		...
		(i === D.Array || i === D.Map) && (r = [{
			name: n === "deconstruct" ? i === D.Array ? "0" : "???" : s,
			id: o,
			type: c,
			optional: l,
			children: u
...
var Ci = /* @__PURE__ */ w((/* @__PURE__ */ S(((e, t) => {
	(function() {
...
function Di(e, t, n, r) {
	...
	return {
		onDragStart(e) {
			...
			let t = e.target;
			Ti(t, "ͼbaseflow-dragging"), i = { source: t }, n?.(t, !0);
		},
		onDragEnter(t) {
			if (!i.source) return;
			let { target: n, folder: r } = e(t.target);
			i.folder !== r && (i.folder && Ei(i.folder, "drop-active"), r && Ti(r, "drop-active"), i.folder = r), i.target !== n && (i.target && Ei(i.target, "drop-active"), n && (Ti(n, "drop-active"), t.preventDefault()), i.target = n);
		},
		onDragOver(e) {
			i.target && e.preventDefault();
		},
		onDragEnd(e) {
			i.source && (n?.(i.source, !1), Ei(i.source, "ͼbaseflow-dragging")), i.target && Ei(i.target, "drop-active"), i.folder && Ei(i.folder, "drop-active"), i = {};
		},
		onDrap() {
...
var K = {
	...
	addClass: Ti,
	removeClass: Ei,
	classNames: (...e) => (0, Ci.default)(...e),
	buildDragHandlers: Di
};
var Oi = "Node";
var q = {
	[D.Any]: "any",
...
var ki = " | null | undefined";
var Ai = [`declare const ${ni}: unique symbol;
declare const ${ri}: string;
interface ${q[D.Time]} {
  [${ni}]: 'Time';
  toString: () => string;
};
...
type ${q[D.Object]} = { [key: string]: any };
type ${q[D.Map]} = { [key: string]: any };
type ${q[D.Array]} = any[];
...
function ${H._date}(value: any): ${q[D.Date]};
function ${H._time}(value: any): ${q[D.Time]};
function ${H._datetime}(value: any): ${q[D.DateTime]};
...
function ji(e) {
	let t = [];
	return R.each(e, (e) => {
		e.outputSchema && t.push(`const ${e.id}: ${Oi}.${e.id};`);
	}), t.join("\n");
}
var J = {
	A: "{⫻",
...
function Ni(e) {
	return !!(O[e.type] && e.name);
}
function Pi(e) {
	return !!(e.value && ne[e.value.source] && e.name);
}
function Fi(e, t, n, r = { current: 0 }, i = "") {
	let a = `${i}const v${r.current++}`;
	if (t) n.push(`${a}: _Iterator_ = ${e.text || "null"};`);
	else {
		let t = `${a}: ${q[e.type]}${e.optional ? ki : ""} = `;
		if (e.text) {
			if (e.source !== k.Template) n.push(`${t}${e.text};`);
			else if (e.text !== "*") {
...
function Ii(e, t, n, r = 0, i = "") {
	let a = "    ".repeat(r), o = G.valueModeIsMapping(e);
	if (Fi(e.value, o, t, n, a), o) {
		t.push(`${a}{`);
		let { text: n, source: r } = e.value;
		if (!n || r !== k.Variable && /\D/.test(n) || new RegExp(ti).test(n) && !i) t.push(`${a}    const ${ei} = undefined;`), t.push(`${a}    const ${V} = undefined;`), t.push(`${a}    const ${z} = undefined;`), t.push(`${a}    const ${B} = undefined;`), i = "";
		else if (/\D/.test(n)) {
			let e = n.replace(new RegExp(ti), i);
			t.push(`${a}    const ${ei} = ${e};`), t.push(`${a}    const {${V}, ${z}, ${B}} = ${U._loopKeyValue}(${ei});`), i = `${e}[0]`;
		} else t.push(`${a}    const ${ei}: number[] = null as any;`), t.push(`${a}    const ${V}: number = null as any;`), t.push(`${a}    const ${z}: number = null as any;`), t.push(`${a}    const ${B}: number = null as any;`), i = "";
	}
	let s = e.children;
	if (s) for (let e = 0, a = s.length; e < a; e++) Ii(s[e], t, n, o ? r + 1 : r, i);
	o && t.push(`${a}}`);
}
function Li(e, t) {
	let n = { current: 0 };
	for (let r in e) {
		let i = e[r];
		i && typeof i == "object" && (Pi(i) ? Ii(i, t, n) : Mi(i) ? Fi(i, !1, t, n, "") : Ni(i) || Li(i, t));
	}
}
function Ri(e) {
	let t = [];
...
function zi(e, t = []) {
	return e && (/\D/.test(e) ? (t.push(`const ${ei} = ${e};`), t.push(`const {${V}, ${z}, ${B}} = ${U._loopKeyValue}(${ei});`)) : (t.push(`const ${ei}: number[] = null as any;`), t.push(`const ${V}: number = null as any;`), t.push(`const ${z}: number = null as any;`), t.push(`const ${B}: number = null as any;`))), t.join("\n");
}
function Bi(e, t, n) {
	let r = [];
	return zi(t, r), Fi(e, n, r), r.join("\n");
}
function Vi(e, t, n, r, i, a) {
	let o = " ".repeat((t - 1) * 2);
	if (n) {
		let n = e[0];
		return n.type === D.Array ? (r.push(`${o}Array<`), Vi(n.children || [], t + 1, !0, r, `${i + "⫻" + (n.optional ? "?" : "!")}0`, n), r.push(`${o}${n.optional ? ki : ""}>`)) : n.type === D.Object || n.type === D.Map ? (r.push(`${o}Array<{`), Vi(n.children || [], t + 1, !1, r, `${i + "⫻" + (n.optional ? "?" : "!")}0`, n), r.push(`${o}}${n.optional ? ki : ""}>`)) : n.optional ? r.push(`${o}(${q[n.type]} | null)[]`) : r.push(`${o}${q[n.type]}[]`), r;
	}
	{
		let n = a?.type === D.Map;
		return e.forEach((e) => {
			...
			let s = e.optional ? ki : "", c = t === 1 ? `${o}type ${e.name} = ` : n ? `${o}[key: string]: ` : `${o}"${e.name.replace(/"/g, "\\\"")}"${e.optional ? "?" : ""}: `;
			if (e.type === D.Object || e.type === D.Map) r.push(`${c}{`), Vi(e.children || [], t + 1, !1, r, a, e), r.push(`${o}}${s};`);
			else if (e.type === D.Array) {
				let n = (e.children || [])[0] || {
					name: "*",
					type: D.Any
				};
				n.type === D.Object || n.type === D.Map ? (r.push(`${c}Array<{`), Vi(n.children || [], t + 1, !1, r, `${a + "⫻" + (n.optional ? "?" : "!")}0`, n), r.push(`${o}}${n.optional ? ki : ""}>${s};`)) : n.type === D.Array ? (r.push(`${c}Array<`), Vi(n.children || [], t + 1, !0, r, `${a + "⫻" + (n.optional ? "?" : "!")}0`, n), r.push(`${o}${n.optional ? ki : ""}>${s};`)) : n.optional ? r.push(`${c}(${q[n.type]} | null)[]${s};`) : r.push(`${c}${q[n.type]}[]${s};`);
			} else r.push(`${c}${q[e.type]}${s};`);
		}), r;
	}
}
function Hi(e) {
	return Vi(e, 1, !1, [], "", null);
}
function Ui(e, t, n, r = 0, i = !1) {
	...
	if (o === "assign") t.push(`${p}${d === k.Template ? "'' as any" : u || "null"}${h}`);
...
function Wi(e) {
	let { variable: t } = li.matchVariable(e.path);
	if (e.value) {
		let n = [];
		return Ui(e.value, n, t), `    ${U._matchVariable}(${t}, ${n.join("\n")});`;
	}
...
var Gi = {
	...
	NodeNamespace: Oi,
	systemDTS: Ai,
	superInputToInspector: Bi,
	propsToInspector: Ri,
	contextToInspector: zi,
	deviceToVariable: ji,
	schemaToDTS: Hi,
	schemaValueToSRC: Wi
};
...
function qi({ locale: e, lang: t, monacoEditorUrl: n, expressionUtils: r }) {
	t && Object.assign(A, t), e && (Ki.locale = e), n && (Ki.monacoEditorUrl = n), r && si(r);
}
function Ji() {
	return Ki.locale;
}
var Yi = { validateContextExpression: 50 };
if (!window["@baseflow/schema"]) {
	...
	class n {
		...
		constructor() {
			...
			this.iframe = e, e.className = "ͼbaseflow-sr-monaco", e.src = Ki.monacoEditorUrl, document.body.appendChild(e), this.proxy = new Promise((e, t) => {
				this.callback = {
					resolve: e,
					reject: t
				};
		...
		init(e) {
			this.tsServer = e, e.ss(Gi.systemDTS.join("\n")), this.callback.resolve({
				tsServer: e,
...
var $i = G.createSchemaValueByModel;
var ea = class {
	_listenerId = 0;
	_listenerMap;
	constructor(e) {
...
no.unshift("<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" aria-hidden=\"true\" focusable=\"false\" width=\"0\" height=\"0\">"), no.push("</svg>");
var ro = no.join("\n");
var io = (0, import_react.createContext)("");
function ao() {
	return (0, import_react.useContext)(io);
}
var oo = (0, import_react.createContext)({});
function so() {
	return (0, import_react.useContext)(oo);
}
oo.Provider;
function Z(e) {
...
var co = function({ className: e, style: t, htmlType: n, children: r }) {
...
var lo = function({ value: e, onChange: t, className: n, placeholder: r }) {
...
var uo = (0, import_react.memo)(({ value: e }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: e }));
var fo = function({ value: e, onChange: t, className: n, block: r, placeholder: i }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
...
var po = (0, import_react.memo)(({ options: e, value: t, onChange: n, className: r }) => {
	let i = Z((e) => {
		e !== t && n?.(e);
	});
...
var mo = function({ value: e, options: t, onChange: n, className: r, block: i }) {
...
var go = function({ value: e, onChange: t, className: n }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
		className: n,
		type: "checkbox",
		checked: !!e,
...
var _o = function({ className: e, style: t, rows: n, value: r, noTrim: i, block: a, onChange: o, onBlur: s }) {
...
var vo = function({ value: e, onChange: t, className: n, placeholder: r }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
		className: n,
		type: "time",
		value: e,
...
var yo = (0, import_react.memo)(function({ payload: e, onClose: t }) {
...
var xo = (0, import_react.memo)(({ payload: e, onClose: t }) => {
	...
	return (0, import_react.useEffect)(() => {
		...
		let t = e.target;
		return r.current = t, t && (i.current &&= (clearTimeout(i.current), 0), bo(t, n.current, e.offset), t.addEventListener("mouseleave", a, !1)), () => {
...
var Co = (0, import_react.memo)(({ payload: e, onClose: t }) => {
	let n = (0, import_react.useRef)(void 0), r = (0, import_react.useRef)(void 0), i = (0, import_react.useRef)(0), a = Z(() => {
		e && !i.current && (i.current = setTimeout(() => {
			t(), i.current = 0;
		}, 300));
	...
	return (0, import_react.useEffect)(() => {
		r.current && r.current.removeEventListener("mouseleave", a, !1);
		let t = e.target;
		return r.current = t, t && (i.current &&= (clearTimeout(i.current), 0), So(t, n.current), t.addEventListener("mouseleave", a, !1)), () => {
			t.removeEventListener("mouseleave", a, !1);
		};
...
var wo = (0, import_react.memo)(() => {
	...
	return (0, import_react.useMemo)(() => {
		...
		children: [
			n ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Co, {
				payload: n,
				onClose: l
			}) : null,
			e ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(yo, {
				payload: e,
				onClose: s
			}) : null,
			i ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(xo, {
				payload: i,
				onClose: c
			}) : null,
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				style: { display: "none" },
				dangerouslySetInnerHTML: { __html: ro }
			})
		]
	});
});
...
var Do = {
	[k.Expression](e, t) {
		if (e) {
			if (t === k.Variable) return e;
			if (t === k.Template) return J.toJSTpl(e);
		}
		return e;
	},
	[k.Template](e, t) {
		if (e) {
			if (t === k.Variable) return J.wrapVariable(e);
			if (t === k.Expression) return "";
		}
		return e;
	},
	[k.Variable](e, t) {
		if (e) {
			if (t === k.Expression) return "";
			if (t === k.Template) return J.extractVariable(e)?.[0] || "";
		}
		return e;
	}
};
function Oo(e, t) {
	let n = t || e?.source || k.Variable, r = e?.text || "", i = "";
	return e?.text ? (t && e.source !== t && (r = Do[t](e.text, e.source)), i = n === k.Variable && r ? J.wrapVariable(r) : r) : i = "", {
		source: n,
		text: r,
		mode: n === k.Expression ? "complex" : "simple",
		code: i
	};
}
function ko(e, t, n, r, i) {
	e = e.trim();
	let a, o;
	if (n.mode === t ? t === "simple" ? e ? (o = J.getSingleVariable(e), o ? a = k.Variable : (a = k.Template, o = e)) : (a = k.Variable, o = "") : (a = k.Expression, o = e) : t === "simple" ? (a = k.Variable, o = "") : (a = k.Expression, o = J.getSingleVariable(e), o ||= J.toJSTpl(e)), r && r !== a && (o = Do[r](o, a), a = r), i === "mapping") {
		if (a === k.Template && /\D/.test(o) || a === k.Expression) o = "", a = k.Variable;
		else if (a === k.Variable) {
			let { fun: e } = li.matchVariable(o);
			e && e !== H._number && e !== H._path && (o = "", a = k.Variable);
		}
	} else if (i === "variable") {
		if (a === k.Template || a === k.Expression) o = "", a = k.Variable;
		else if (a === k.Variable) {
			let { fun: e } = li.matchVariable(o);
			e && e !== H._path && (o = "", a = k.Variable);
		}
	}
	return {
		source: a,
...
function Mo(e) {
	...
	return n[n.length - 1] && n[n.length - 1].nodeName === "BR" && n.length > 1 && (n = n.splice(0, n.length - 1)), n.forEach((e) => {
		if (e.nodeName === "#text") t += e.nodeValue;
			...
			n.length === 1 && n[0].nodeName === "BR" || e.previousSibling && e.previousSibling.nodeName === "BR" ? t += Mo(e) : t += `\n${Mo(e)}`;
		} else t += Mo(e);
	}), t.replace(/\u00A0/g, " ").trim();
}
function No(e, t) {
...
function Fo(e, t) {
	...
	n.contentEditable = "false", n.className = "ͼbaseflow-TplEditor__var", n.innerHTML = `<i class="act"></i>${t.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\s/g, "&nbsp;")}`, No(e, n);
}
function Io(e) {
	e.stopPropagation(), e.preventDefault();
	let t = e.clipboardData?.getData("text/plain");
	if (t) {
		let e = jo(t), n = window.getSelection();
...
var Ro = (0, import_react.memo)(({ path: e, args: t, onChange: n }) => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [i, /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.Button, {
				size: "small",
				onClick: o,
				children: A.ButtonCancel
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.Button, {
				size: "small",
				type: "primary",
				onClick: s,
				children: A.ButtonSubmit
			})]
		})]
	});
});
var zo = (0, import_react.memo)(({ tag: e, target: t, sourceType: n, onSubmit: r }) => {
	let { text: i, fun: a, variable: o, args: s, isAssert: c } = (0, import_react.useMemo)(() => li.matchVariable(e.textContent || ""), [e.textContent]), l = Z(() => {
		...
		n === H._path ? Q.popup({
			content: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ro, {
				path: o,
				args: s,
				onChange: d
			}),
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [n !== "variable" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.Switch, {
				...
				value: c,
				onChange: l
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { children: A.nullAssert })]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "funs",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: A.setPath }),
				...
				n !== "variable" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { name: Y.Help }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: A.convertTo }),
...
var Vo = (0, import_react.forwardRef)(({ value: e = "", onChange: t, onBlur: n, onFocus: r, onClickVariable: i, placeholder: a, sourceType: o }, c) => {
	...
	let b = Z(() => {
		...
		let { variable: t } = li.matchVariable(e.target.nodeName === J.VarTag ? e.target.textContent : "");
		t = t.replace(/!$/, ""), t && i?.(t);
	}), w = Z((e) => {
		if (!_.current) {
	...
	return (0, import_react.useImperativeHandle)(c, () => ({ insertVariable: w }), [w]), (0, import_react.useEffect)(() => {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			...
			children: a || A.requiredPrompt
		}) : null]
	});
});
var Ho = (0, import_react.memo)(Vo);
var Uo = (0, import_react.memo)(({ value: e, onChange: t, options: n }) => {
...
var qo = {
	[D.Bool]: (e, t) => !e || e === "true" || e === "false" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.Select, {
		valueNotBeEmpty: !0,
		variant: "borderless",
		block: !0,
		className: "nativeInput",
	...
	[D.Date]: (e, t) => !e || !Go(e) ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.DatePicker, {
		variant: "borderless",
		block: !0,
		className: "nativeInput",
		placeholder: A.selectPrompt,
	...
	[D.Time]: (e, t) => !e || !Ko(e) ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.TimePicker, {
		variant: "borderless",
		block: !0,
		className: "nativeInput",
		placeholder: A.selectPrompt,
	...
	[D.DateTime]: (e, t) => !e || !Go(e) ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.DatePicker, {
		variant: "borderless",
		block: !0,
		showTime: !0,
		className: "nativeInput",
	...
	[D.String]: (e, t) => t ? (t = t.map((e) => e.label ? e : {
		...e,
		label: e.value
	...
	[D.Array]: (e, t) => t ? (t = t.map((e) => e.label ? e : {
		...e,
		label: e.value
	}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Uo, { options: t })) : null
};
function Jo(e, t) {
	return e === D.String || e === D.Number ? t ? qo[D.String] : void 0 : e === D.Array ? t ? qo[D.Array] : void 0 : qo[e];
}
var Yo = (0, import_react.forwardRef)((t, n) => {
	let { value: r, onChange: i, onClickVariable: a, dataType: o, nativeRenderOptions: s, placeholder: c, sourceType: g } = t, [_, v] = (0, import_react.useState)(!0), y = t.nativeRender || Jo(o, s), b = (0, import_react.useRef)(null), x = (0, import_react.useMemo)(() => {
		let t = y ? y(r, s) : null;
	...
	return (0, import_react.useImperativeHandle)(n, () => ({ insertVariable: C }), [C]), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [!_ || !x ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ho, {
		ref: b,
		value: r,
		onChange: i,
		onClickVariable: a,
...
var Zo = (0, import_react.forwardRef)((e, t) => {
	let n = so(), [r] = (0, import_react.useState)(Xi.createUID()), i = (0, import_react.useRef)(null), o = (0, import_react.useRef)(""), { style: c, variant: p, nativeRender: g, optional: _, nativeRenderOptions: v, onChange: y, sourceType: b, error: x, hideError: S, dataTypeEditable: C, context: w = "", runtime: T = "expression", dataType: E = D.String } = e, ee = e.variableFilter, O = T === "script" ? k.Expression : e.dataSource, ne = b === "mapping" ? e.placeholder || A.loopContextPrompt : e.placeholder, re = b === "mapping" || e.hideIcon, [ie, ae] = (0, import_react.useState)(!1), [j] = (0, import_react.useState)(new Eo()), [M, oe] = (0, import_react.useState)(""), se = Z((e) => {
		M !== e && oe(e);
	}), ce = (0, import_react.useMemo)(() => {
		if (e.height) return { minHeight: e.height };
	}, [e.height]), N = (0, import_react.useMemo)(() => K.classNames("ͼbaseflow-SuperInput", e.className, p, {
		active: ie,
		hideIcon: re,
		hasError: !S && (x || M)
	}), [
		...
		p,
		ie
	]), le = (0, import_react.useRef)(void 0);
	(0, import_react.useMemo)(() => {
		le.current = E;
	}, [E]);
	let P = (0, import_react.useMemo)(() => Oo(e.value, O), [e.value, O]), ue = Z((t) => {
		if (P.mode !== t) {
			let { source: n, text: r } = ko(P.code, t, P, O, b);
		...
		if (P.code !== t) {
			let { source: n, text: r } = ko(t, P.mode, P, O, b);
			if (n !== e.value?.source || r !== e.value?.text) {
				...
				C && n === k.Variable && le.current && (e.type = le.current), y?.(e);
			}
		}
	}), fe = Z((e) => {
		y?.({
	...
	}), pe = (0, import_react.useCallback)((e, t) => {
		le.current = t, i.current?.insertVariable(e);
	}, []), me = Z(() => P.mode), he = Z(() => E), ge = Z(() => w), _e = Z(() => T), ve = Z(() => ee), ye = Z(() => e.value), be = (0, import_react.useCallback)(() => o.current, []), xe = Z(() => e.brand), Se = (0, import_react.useCallback)((e) => {
		e || (o.current = ""), ae(e);
	}, []), [Ce] = (0, import_react.useState)({
		setMode: ue,
		getMode: me,
		setText: fe,
		insertVariable: pe,
		...
		getRuntime: _e,
		setActive: Se,
		getVariableFilter: ve,
		addListener: j.addListener.bind(j)
	});
	(0, import_react.useImperativeHandle)(t, () => Ce, [Ce]);
	let we = Z(() => {
		ie || n.setActivedSuperInput(Ce);
	}), Te = Z(() => {
		if (!_ && !P.text) {
			se(A.requiredPrompt);
			return;
		}
		let e = Gi.superInputToInspector({
			type: E,
			optional: _,
			source: P.source,
			text: P.text
		}, w, b === "mapping");
		e ? n.validateSuperInput(`${r}`, e, se) : se("");
	}), Ee = Z((e) => {
		o.current = e, j.dispatch("variableClick", {
			variable: e,
			target: Ce
		});
	}), De = Z((e) => {
		j.dispatch("expressionClick", {
			expression: e,
			target: Ce
		});
	}), Oe = Z((e) => {
	...
	}), ke = (0, import_react.useMemo)(() => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", {
	...
	return (0, import_react.useEffect)(() => {
		...
		n.getActivedSuperInput() === Ce && n.setActivedSuperInput(null);
	}, []), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: K.classNames(N, P.mode),
		"data-error": x || M || void 0,
		style: c,
		children: [
			!O && !b && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "ͼbaseflow-SuperInput__modeSwitch",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: K.classNames("item", { on: P.mode === "simple" }),
					onClick: () => ue("simple"),
					children: A.simpleValue
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: K.classNames("item", { on: P.mode === "complex" }),
					onClick: () => ue("complex"),
					children: A.complexValue
				})]
			}),
			!re && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, {
				className: K.classNames("dataType", { editable: C }),
				name: E,
				onClick: Ae
			}),
			!S && (x || M) && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "dataError",
				title: x || M,
				onClick: (e) => {
					Zi.clipboard.write(e.currentTarget.textContent), Zi.message.success(A.copied);
				},
				children: x || M
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: K.classNames("ͼbaseflow-SuperInput__input", P.mode),
				"data-baseflow-role": K.domRoles.SuperInput,
				style: ce,
				onClick: we,
				children: P.mode === "complex" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ao, {
					onChange: de,
					text: P.code,
					onClickExpression: De
...
function es(e) {
	switch (e) {
		case D.String: return [
			...
			{
				label: A.containsString,
				value: "containsString"
			},
			{
				label: A.notContainsString,
		...
		case D.DateTime: return [
			...
			{
				label: A.laterOrEqual,
				value: "laterOrEqual"
			},
			{
				label: A.earlierOrEqual,
		...
		case D.Array: return [
			...
			{
				label: A.containsItem,
				value: "containsItem"
			},
			{
				label: A.notContainsItem,
...
function ts(e, t) {
	return t === "in" || t === "notIn" ? D.Array : t === "hasKey" || t === "notHasKey" ? D.String : t === "containsItem" || t === "notContainsItem" ? D.Any : e;
}
var ns = (0, import_react.memo)(({ value: e, onChange: t }) => {
	let n = Z(() => {
		t(e === "and" ? "or" : "and");
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
	...
	(0, import_react.useMemo)(() => {
		if (e.value !== n.current.value) {
			...
			n.current._value = t, n.current.__value = t, t.groups.forEach((e) => {
				e.items.forEach((e) => {
					e.operatorOptions ||= es(e.source.type);
				});
			});
			...
			children: [e.items.map((r, i) => (r.key ||= t.current++, /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "title",
							children: i === 0 ? "WHEN" : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ns, {
								value: e.relation,
								onChange: (e) => o(n, e)
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Qo, {
							dataTypeEditable: !0,
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "btn",
						onClick: () => s(n),
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { name: Y.PlusCircle }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: A.addGroup })]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "btn",
						onClick: () => c(n),
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { name: Y.MinusCircle }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: A.delGroup })]
					}),
					i.groups.length > 1 && n < i.groups.length - 1 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "group-relation",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ns, {
							value: i.relation,
							onChange: a
						})
					})
...
var ss = (0, import_react.memo)(({ value: e, onChange: t }) => {
	...
	return e === void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				checkedChildren: A.scriptsMode,
				unCheckedChildren: A.scriptsMode,
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "label",
				children: A.conditionScripts
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "input",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Qo, {
					height: 100,
					hideIcon: !0,
					runtime: "script",
					brand: "variable",
		...
		}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(is, {
			value: e,
			onChange: t || $o
		})]
	});
});
var cs = (e) => typeof e == "boolean" || e instanceof Boolean;
var ls = (e) => typeof e == "number" || e instanceof Number;
var us = (e) => typeof e == "bigint" || e instanceof BigInt;
var ds = (e) => !!e && e instanceof Date;
var fs = (e) => typeof e == "string" || e instanceof String;
var ps = (e) => Array.isArray(e);
var ms = (e) => typeof e == "object" && !!e;
var hs = (e) => !!e && e instanceof Object && typeof e == "function";
function gs(e, t) {
...
function vs(e) {
	...
	let C = y ? p.collapseIcon : p.expandIcon, w = y ? p.ariaLables.collapseJson : p.ariaLables.expandJson, T = u + 1, E = i.length - 1, ee = (e) => {
		y !== e && (!_ || _({
			level: u,
			value: r,
			field: t,
	...
	return /*#__PURE__*/ (0, import_react.createElement)("div", {
		...
		onClick: O,
		onKeyDown: D
	}, gs(t, p.quotesForFieldNames), ":") : /*#__PURE__*/ (0, import_react.createElement)("span", { className: p.label }, gs(t, p.quotesForFieldNames), ":")), /*#__PURE__*/ (0, import_react.createElement)("span", { className: p.punctuation }, o), y ? /*#__PURE__*/ (0, import_react.createElement)("ul", {
		id: S,
		role: "group",
		className: p.childFieldsContainer
	}, i.map((e, t) => /*#__PURE__*/ (0, import_react.createElement)(Cs, {
		key: e[0] || t,
		field: e[0],
		value: e[1],
		style: p,
...
function ys(e) {
	...
	return /*#__PURE__*/ (0, import_react.createElement)("div", {
		...
		role: "treeitem",
		"aria-selected": void 0
	}, (t || t === "") && /*#__PURE__*/ (0, import_react.createElement)("span", { className: o.label }, gs(t, o.quotesForFieldNames), ":"), /*#__PURE__*/ (0, import_react.createElement)("span", { className: o.punctuation }, r), /*#__PURE__*/ (0, import_react.createElement)("span", { className: o.punctuation }, i), !a && /*#__PURE__*/ (0, import_react.createElement)("span", { className: o.punctuation }, ","));
}
function bs(e) {
...
function Ss(e) {
	...
	return r === null ? (o = "null", s = i.nullValue) : r === void 0 ? (o = "undefined", s = i.undefinedValue) : fs(r) ? (o = _s(r, !i.noQuotesForStringValues, i.stringifyStringValues), s = i.stringValue) : cs(r) ? (o = r ? "true" : "false", s = i.booleanValue) : ls(r) ? (o = r.toString(), s = i.numberValue) : us(r) ? (o = `${r.toString()}n`, s = i.numberValue) : o = ds(r) ? r.toISOString() : hs(r) ? "function() { }" : r.toString(), /*#__PURE__*/ (0, import_react.createElement)("div", {
		className: i.basicChildStyle,
		role: "treeitem",
		"aria-selected": void 0
	}, (t || t === "") && /*#__PURE__*/ (0, import_react.createElement)("span", { className: i.label }, gs(t, i.quotesForFieldNames), ":"), /*#__PURE__*/ (0, import_react.createElement)("span", { className: s }, o), !a && /*#__PURE__*/ (0, import_react.createElement)("span", { className: i.punctuation }, ","));
}
function Cs(e) {
	let t = e.value;
	return ps(t) ? /*#__PURE__*/ (0, import_react.createElement)(xs, Object.assign({}, e)) : ms(t) && !ds(t) && !hs(t) ? /*#__PURE__*/ (0, import_react.createElement)(bs, Object.assign({}, e)) : /*#__PURE__*/ (0, import_react.createElement)(Ss, Object.assign({}, e));
}
var $ = {
...
var Ts = () => !0;
var Es = (e) => {
	let { data: t, style: r = ws, shouldExpandNode: i = Ts, clickToExpandNode: a = !1, beforeExpandChange: o, compactTopLevel: s, ...c } = e, l = (0, import_react.useRef)(null);
	return /*#__PURE__*/ (0, import_react.createElement)("div", Object.assign({ "aria-label": "JSON view" }, c, {
		className: r.container,
		ref: l,
		role: "tree"
	}), s && ms(t) ? Object.entries(t).map((e) => {
		let [t, s] = e;
		return /*#__PURE__*/ (0, import_react.createElement)(Cs, {
			key: t,
			field: t,
			value: s,
			style: {
	...
	}) : /*#__PURE__*/ (0, import_react.createElement)(Cs, {
		value: t,
		style: {
			...ws,
			...r
...
var Ds = (0, import_react.memo)(({ data: e, shouldExpandNode: t }) => {
	let n = (0, import_react.useMemo)(() => ({
		...ws,
...
var Os = (0, import_react.memo)(({ value: e, onChange: t, ...n }) => {
...
var ks = (0, import_react.memo)(({ value: e, onChange: t, labelPlaceholder: n = "label", valuePlaceholder: r = "value", hideLabel: i, className: a, variant: o }) => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [e?.length ? e.map((e, t) => {
			...
			return a || (a = ++s.current, c.current.set(e, a)), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "item",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Os, {
...
var As = (0, import_react.memo)(({ onClose: e, item: t }) => {
	let [n, r] = (0, import_react.useState)(W.schemaModelToXml(t)), [i, a] = (0, import_react.useState)(""), o = Z(() => {
		...
		Zi.clipboard.write(e).then(() => Zi.message.success(A.alreadyToClipboard));
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "title",
								children: `${A.jsonValue}:`
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "link",
								onClick: () => s(i),
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { name: Y.Copy }), A.copy]
							})]
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "ft",
				children: A.exportSchemaTips
			})
		]
	});
});
function js({ namespace: e, looseLeaf: t, node: n, parent: r, index: i, isFirst: a, isLast: s, parentLevel: c, parentPath: l, parentIds: u, parentIsEnd: d, parentIsFinish: f, attributeRender: p, context: g, contextValueRender: _, nodeRender: v, folderRender: y }) {
	let b = {
		node: n,
		parent: r,
		index: i,
	...
	}, x = p ? p(b) : void 0, S = _ ? _(b) : void 0, C = !b.isLeaf && n.disabled !== "all", w = C && n.children.map((r, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ms, {
		namespace: e,
		looseLeaf: t,
		context: g,
...
function Ns(e) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: K.classNames("ͼbaseflow-SimpleTree", r && `ͼbaseflow-${r}`),
		style: n,
		children: f !== "all" && d.map((e, n) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ms, {
			namespace: r,
			looseLeaf: i,
			context: a,
			contextValueRender: s,
...
var Ps = (0, import_react.memo)(Ns);
var Fs = ({ className: e, children: t }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", {
	className: K.classNames("ͼbaseflow-FormLayout", e),
	children: t
});
var Is = (0, import_react.memo)(({ value: e, onChange: t, error: n }) => {
	...
	let p = Z(() => {
		...
		if (e) try {
			...
			console.error(e), d(A.jsonValueTips);
		}
	}), v = Z(() => {
		let e = a.trim();
		if (e) try {
			...
			console.error(e), d(A.schemaValueTips);
		}
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
								className: "format",
								onClick: _,
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { name: Y.Loop }), A.format]
							})
						]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Q.TextArea, {
						className: "input",
			...
			className: "ft",
			children: A.importSchemaTips.split("\n").map((e) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: e }, e))
		})]
	});
});
var Ls = [{
	label: A.createdFromManual,
	value: "manual"
}, {
	label: A.createdFromDSL,
	value: "import"
}];
var Rs = (0, import_react.memo)(({ onCancel: e, onSubmit: t, item: n, place: r, target: i, targetPath: a, parent: o, nameExists: c, editableFilter: l }) => {
	let d = o?.type === D.Array || o?.type === D.Map, [p, g] = (0, import_react.useState)(), _ = !!(!p || p.name === void 0 || p.name) && !d && !!o, v = !!(!p || p.type === void 0 || p.type), { typeOptions: y, typeMap: b } = (0, import_react.useMemo)(() => {
		let e = te, t = p?.type;
		return t === !1 ? e = [{
			value: n.type || D.String,
			label: O[n.type || D.String]
		...
		return t === !1 ? e = [{
			value: n.subType || D.String,
			label: O[n.subType || D.String]
		}] : Array.isArray(t) && (e = t.map((e) => ({
			label: O[e],
			value: e
		}))), {
		...
		let e = p?.name, t;
		return e === !1 ? t = (e) => {
			if (e !== n.name) return re(A.nameRestricted, { name: e });
		} : typeof e == "function" && (t = e), t;
	}, [p?.name, n.name]), [w, T] = (0, import_react.useState)("manual"), [E, ee] = (0, import_react.useState)(""), [k, ne] = (0, import_react.useState)(""), ie = Z((e) => {
		e === "import" && (E || ee(W.schemaModelToXml(n))), T(e);
	}), ae = Z((e) => {
		if (!e) return A.nameIsRequired;
		if (!_ && e !== n.name) return re(A.nameRestricted, { name: e });
		if (e !== n.name && c[e]) return re(A.nameIsRepeat, { name: e });
	}), [j, M] = (0, import_react.useState)(() => ({
		type: n.type,
		name: n.name,
		label: n.label,
		tips: n.tips,
	...
	let oe = Z((e) => {
		...
		M({
			...j,
			type: e,
			subType: e === D.Array || e === D.Map ? n.subType || D.String : void 0
		});
	}), ue = Z((e) => {
		M({
			...j,
		...
		if (!e.trim()) return A.schemaValueTips;
		let t, n;
		try {
		...
		if (!t) return typeof n == "string" ? n : A.schemaValueTips;
		let r = ae(t.name) || C?.(t.name) || "";
		if (r) return r;
		if (!b[t.type]) return re(A.typeRestricted, { type: O[t.type] });
		let i = t.type === D.Array || t.type === D.Map ? t.children[0].type : void 0;
		return i && !S[i] ? re(A.typeRestricted, { type: O[i] }) : t;
	}), fe = Z(() => {
		let e;
		if (w === "import") {
			let t = de(E);
			typeof t == "string" ? ne(t) : e = t;
		} else {
			let { type: t, name: n, label: a, tips: o, optional: s, subType: c } = j, l = j.enums, u = ae(n) || C?.(n) || "";
			if (u !== j.nameError && M({
				...j,
				nameError: u
			}), !u && (e = {
				...
				tips: o,
				optional: s
			}, r === "replace" && i.type === t && (t === D.Object || (t === D.Array || t === D.Map) && i.children?.[0].type === c) && (e.children = i.children), l && (l = l.filter((e) => e.value), l.length || (l = void 0), e.enums = l), (t === D.Array || t === D.Map) && !e.children)) {
				let t = {
					type: c || D.String,
					name: "*"
				};
				e.children = [t], (t.type === D.Array || t.type === D.Map) && (t.children = [{
					type: D.String,
					name: "*"
				}]);
			}
	...
	return (0, import_react.useEffect)(() => {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "hd",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, { name: r === "replace" ? Y.Edit : r === "sub" ? Y.AddSub : Y.AddNext }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [r === "replace" ? A.update : r === "sub" ? A.insertChild : A.insertNext, i ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", {
					className: "tips",
					children: `(${i.name})`
				}) : null] })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [p?.import !== !1 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				}), w === "import" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Is, {
					value: E,
					onChange: ee,
					error: k
					...
					children: [
						_ && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Os, {
								value: j.name,
								className: `ͼbaseflow-input${j.nameError ? " ͼbaseflow-error" : ""}`,
								placeholder: "name",
								onChange: le,
								block: !0
							}), !!j.nameError && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "item-error",
								children: j.nameError
							})]
						})] }),
						...
						(j.type === D.String || j.type === D.Number) && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "item-label",
							children: A.useEnum
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "item-content",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ks, {
								value: j.enums,
								onChange: oe
							})
						})] }),
						(j.type === D.Array || j.type === D.Map) && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "item-label required",
							children: A.subType
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "item-content",
...
var Bs = (0, import_react.memo)(({ className: e, node: t, parent: n, nodePath: r, labelRender: i, toolsFilter: a, onDelItem: o, onEditItem: c, onExportItem: l }) => {
	let d = n && (n.type === D.Array || n.type === D.Map), [p, g] = (0, import_react.useState)(), _ = (0, import_react.useMemo)(() => W.toSchemaTitle(t, n, i), [
		i,
		t,
		n
	]), v = {
		addNext: n && !d ? {
			...
			btn: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, {
				...
				button: !0,
				title: A.insertNext,
				onClick: () => c(zs, "next", t, r, n)
			}, "addNext")
		} : !1,
		addSub: t.type === D.Object && {
			key: "addChild",
			btn: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(X, {
				name: Y.AddSub,
				button: !0,
				title: A.insertChild,
				onClick: () => c(zs, "sub", t, r, t)
			}, "addChild")
		},
		edit: {
			key: "edit",
...
var Hs = (0, import_react.memo)((e) => {
	let t = e.renderRoot === void 0, n = e.defaultValue || Vs, r = (0, import_react.useRef)(null), i = Z((e) => {
		Q.modal(e ? {
			content: e,
			mask: "closeAble"
		} : null), r.current = e;
	...
	let _ = Z((e) => {
		...
		c && (e = {
			...e,
			subType: c.type
		}), i(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Rs, {
			item: e,
			parent: o,
			target: n,
			targetPath: r,
		...
		i(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(As, {
			item: e,
			onClose: a
		}));
	}), w = Z(({ node: e, parent: t, path: n }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Bs, {
		node: e,
		nodePath: n,
		parent: t,
		onDelItem: v,
	...
	return (0, import_react.useEffect)(() => () => {
		...
		children: e.value ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ps, {
...
var Us = (0, import_react.memo)(({ node: e, parent: t, labelRender: n, onSelect: r }) => {
...
var Ws = (0, import_react.memo)(({ className: e, schema: t, onSwitch: n, onSelected: r, labelRender: i }) => {
	let a = Z((e) => {
		r?.(e.id, e.type);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ps, {
			renderRoot: !0,
			source: t,
			nodeRender: s,
			folderRender: o
...
var Gs = (0, import_react.memo)(({ node: e, parent: t, labelRender: n }) => {
...
var Ks = (0, import_react.memo)(({ className: e, schema: t, labelRender: n }) => {
	...
	let a = Z((e) => {
		...
		className: "ͼbaseflow-folder",
		onClick: () => a(e)
	})), s = Z(({ node: e, parent: t }) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Gs, {
		labelRender: n,
		parent: t,
		node: e
	}));
...
var qs = (0, import_react.memo)(({ className: e = "", value: t = "", title: n, onChange: r }) => {
...
var Js = (0, import_react.memo)(({ dataType: e, mode: t, onChange: n }) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
	/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
	...
	/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
		...
		children: A.deconstruct
	}),
	(e === D.Array || e === D.Map) && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
		className: K.classNames("item", { on: t === "mapping" }),
		onClick: () => t !== "mapping" && n("mapping"),
		children: A.mapping
	})
] }));
var Ys = (0, import_react.memo)(({ className: e, node: t, index: n, schema: r, parent: i, parentSchema: a, labelRender: o, onItemNameChange: s, onItemValueChange: c, onItemModeChange: l, onAddItem: d, onDelItem: f, inputPropsRender: p }) => {
	let g = r.type, _ = r.optional, v = g === D.Object || g === D.Array || g === D.Map, y = g === D.Array || g === D.Map, b = i ? i.value.type : void 0, x = i ? G.getValueMode(i) : void 0, S = b === D.Array || b === D.Map, C = G.getValueMode(t), w = ao(), T = (0, import_react.useMemo)(() => W.toSchemaTitle(r, a, o), [
		o,
		a,
		r
	]), E = {
		...
		l(t, e);
	}), O = Z((e) => c(t.id, e)), te = Z((e) => {
		if (!i) return;
		if (!e) {
			Zi.message.error(A.requiredPrompt);
			return;
		}
		b === D.Array && (e = `${Number(e) || 0}`);
		let n = i.children || [];
		for (let t of n) if (t.name === e) {
			Zi.message.error(re(A.hasExist, { item: e }));
			return;
		}
		s(e, t, i);
	}), k = [
		...
		E.addSub,
		E.delete
	].filter(Boolean), ne = (0, import_react.useMemo)(() => {
		if (r.enums) return r.enums;
		if (r.type === D.Array) return r.children?.[0]?.enums;
	}, [r]), ie = p?.(r);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					S && x === "deconstruct" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(qs, {
						value: t.name,
						onChange: te,
					...
					(v || k.length > 0) && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "item-tools",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "ͼbaseflow-SchemaValueForm__itemTools",
							children: [v && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Js, {
								dataType: g,
								mode: C,
								onChange: ee
							}, "ModeSwitch"), k]
...
var Zs = (0, import_react.memo)(({ schema: e = Xs, value: t, labelRender: n, onChange: r, inputPropsRender: i, showRootTools: a, variant: o }) => {
	let [, s] = (0, import_react.useState)(0), c = (0, import_react.useRef)({}), l = Z((e) => {
		...
		if (c.current.schema === Xs) c.current.value !== void 0 && r?.(void 0);
		else {
	...
	(0, import_react.useMemo)(() => {
		...
		if (c.current.matchError = e === Xs ? "Schema is undefined" : void 0, n || i) {
			let n = c.current.schemaTree;
			if (c.current.value = t, !t) c.current.valueTree = G.createSchemaValueTreeByModel(n, void 0, void 0, !0);
			else {
				let r = e === Xs ? "Schema is undefined" : G.matchSchemaValueByModel(n, t);
	...
	let g = c.current.schemaMap, _ = c.current.valueTree, v = Z((e) => {
		...
		let n = _;
		if (t.value.type === D.Array) {
			let r = (t.children || []).slice(e);
			r.shift(), r.length && (r = r.map((t, n) => ({
				...t,
				name: `${n + e}`
			})), r = G.toSchemaValueTree(r, t.id)), n = R.produce(_, (n) => {
				if (t.id === n.id) return n.children.splice(e, n.children.length - e, ...r), n.children.length || (n.value.source = k.Variable, n.value.text = "", n.children = void 0), !0;
			});
		} else n = R.produce(_, (n) => {
			if (t.id === n.id) return n.children.splice(e, 1), n.children.length || (n.value.source = k.Variable, n.value.text = "", n.children = void 0), !0;
		});
		l(n);
		...
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ys, {
		...
		if (G.getValueMode(e) === "mapping") {
			let n = li.getValidContextRef(e.value);
			return new RegExp(ti).test(n) && (n = t && /\D/.test(t) ? n.replace(z, `${t}[0]`) : ""), n;
		}
	}), ee = Z(({ node: e, isLast: t, isLeaf: n }) => {
		if (t && n) return { className: "ͼbaseflow-last" };
	});
	return c.current.schema === Xs && !c.current.value ? null : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [c.current.matchError && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "tips",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: A.invalidTips }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "msg",
		...
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Ps, {
...
var Qs = {};
var $s = (0, import_react.memo)(({ locale: e = "en", lang: t, widgets: n, expressionUtils: r, children: i }) => ((0, import_react.useMemo)(() => {
	t && qi({ lang: t }), e && qi({ locale: e }), r && qi({ expressionUtils: r }), n && Qi(n), n && To(n);
}, []), (0, import_react.useEffect)(() => {
	Xi.createValidateProvider(), Object.assign(Qs, Zi, Q);
}, []), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [i, /* @__PURE__ */ (0, import_jsx_runtime.jsx)(wo, {})] })));
var ec = [{ required: !0 }];
//#endregion
export { Qs as BaseWidgets, ss as ConditionSelector, D as DataType, Ds as JsonView, ks as KeyValues, T as NodeType, $s as RenderConfigProvider, ec as RequiredRule, Hs as SchemaModelForm, Ws as SchemaSelector, Ks as SchemaShow, Zs as SchemaValueForm, Qo as SuperInput, X as SvgIcon, k as ValueSource, Le as XMLBuilder, Zn as XMLParser, $i as createSchemaValueByModel, Ji as getLocale };
